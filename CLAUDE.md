...
2. For each story with a `due_date`, checks it against three fixed windows, `NOTICE_WINDOWS = (("2d", 2), ("1d", 1), ("0d", 0))` (days-before-due-date, with a short string label used for dedup — `0` means the story is due today). `_build_text` special-cases `window == "0d"` to say "vence HOY" instead of repeating a date that's already today's.
...
- **Plain text while awaiting login** — a third case handled inside `CommandDispatcher.dispatch()` itself (checked first, before command parsing): if a message is plain text (not a command) and `ConversationStateService.consume_awaiting_login(chat_id)` returns a `message_id` (not `None` — set when the user taps "🔐 Iniciar sesión"), the text is routed straight into the existing `LoginCommand.handle(update, text, prompt_message_id)` — the exact same code path as `/login <email> <password>`, plus that extra id. This is the one deliberate exception to "commands are matched by name": it lets a button press collect free-text credentials without a second command-handling abstraction, and without duplicating `LoginCommand`'s logic. The `prompt_message_id` is what lets `LoginCommand` delete the "Mandame tu correo y contraseña..." prompt message on a **successful** login (see the menu system section below) — the `/login <email> <password>` slash-command call site never has one, so it passes `None` by omission and that cleanup is simply skipped there, since there's no prompt message to clean up.
...
`CommandDispatcher.dispatch()` also logs every inbound `update.message` (via `telegram/services/message_log.py`'s `log_message`) as the very first thing it does, before any branching — this is the single choke point for messages the *user* sends, and it's what makes those messages deletable later (see the message log section below). `CallbackDispatcher` doesn't need an equivalent: a `callback_query`'s `message` is always something the bot already sent (and therefore already logged when it was sent), never new user content.

### Menu system (`services/menu_content.py`, `services/conversation_state.py`)

`MenuContentService` is the single source of truth for every menu's text + `inline_keyboard`, keyed by login state (`session.get(TaigaSession, chat_id) is not None`, checked directly — there's no separate "is logged in" boolean flag). `/start` and the `menu:root` callback both render through it, so the two entry points (typing `/start` vs. tapping "⬅️ Volver") never drift out of sync. `ConversationStateService` is a small in-memory (not DB-backed) TTL map — fine for a short-lived "awaiting credentials" window; it does not need to survive a restart. Its map value is an `AwaitingLogin` `NamedTuple` (`expires_at`, `prompt_message_id`), not a bare timestamp — `mark_awaiting_login(chat_id, prompt_message_id, ttl_seconds=300)` stores both, and `consume_awaiting_login(chat_id) -> int | None` pops and returns the `prompt_message_id` if the entry existed and hadn't expired, `None` otherwise. `AuthLoginCallback` calls `mark_awaiting_login` with the id of the message it just edited into the login prompt (`update.callback_query.message.message_id`), which is what lets that exact message be found and deleted later.
...
On the button-triggered path only (`prompt_message_id is not None`), a **successful** login also deletes that original prompt message ("Mandame tu correo y contraseña..." with its "❌ Cancelar" button), so it doesn't linger on screen once the new menu message has been sent. That delete is wrapped in its own `try/except httpx.HTTPStatusError`, nested *inside* the success branch of the outer `try` — not sharing the outer `except httpx.HTTPStatusError` that maps a failed `auth_service.login()` call to "revisá tus credenciales" — so a failure to delete an already-gone prompt message can never be mistaken for a failed login. On a failed login the prompt message is deliberately left alone (still showing "❌ Cancelar", which routes to `menu:root`/`MenuRootCallback` and resets it) — only the plaintext-credentials message gets cleaned up, same as always.
...
### Message log & logout (`telegram/models.py`, `telegram/services/message_log.py`)

Telegram's Bot API has no endpoint to list a chat's past messages, so the only way to later delete "everything in this chat" is to have recorded every `message_id` while it was still fresh. `telegram/models.py` (the one model file outside `taiga/models.py` — this table is genuinely Telegram-only, not a Taiga concern) holds `TelegramMessageLog` (`chat_id`, `message_id`, indexed on `chat_id` since "all messages for this chat" is the only query pattern that matters here). `log_message(session, chat_id, message_id)` inserts a row and commits; it's called after essentially every `send_message` in the codebase (`start.py`, `projects.py`, `pendings.py`, `login.py`'s remaining plain-text replies, `webhook_notification_service.py`'s per-recipient and admin notifications) **and** once per inbound user message via the dispatcher hook above. Two sends are deliberately *not* also logged here: the due-date reminder (`due_date_reminder_service.py`) and the newly-assigned notification (`webhook_notification_service.py`) already persist their own `message_id` in `TaigaDueDateReminder` / `TaigaAssignmentNotification` for their own dedup/cleanup purposes — duplicating that into `TelegramMessageLog` too would just be redundant bookkeeping.

`clear_chat_history(session, client, chat_id)` is what a logout calls: it unions the message ids from all three tables (`TelegramMessageLog`, `TaigaAssignmentNotification`, `TaigaDueDateReminder`) for that `chat_id`, calls `TelegramClient.delete_message` on each — wrapped in `try/except httpx.HTTPStatusError: pass`, same as the assignment-notification cleanup, since Telegram refuses to delete sufficiently old messages and a failure here must never block the rest — then deletes the matching rows from all three tables. Both `LogoutCommand` and `AuthLogoutCallback` call it, then show a fresh root menu: `LogoutCommand` can just delegate to `StartCommand.handle(update, "")` (its `update.message` is a real message, so this works unmodified), but `AuthLogoutCallback` runs on a callback update with no `update.message`, so it replicates `build_root_menu(False)` + `send_message` directly instead — the same small pattern already repeated in a few other files, not worth extracting into a shared helper for two lines. Note `AuthLogoutCallback` no longer edits the tapped menu message in place: `clear_chat_history` deletes it (it was already logged whenever it was first sent), so the post-logout menu has to be a new message.

A message sent before this feature shipped was never logged, so it's permanently outside what any future logout can clean up — there is no way to retroactively discover it.
