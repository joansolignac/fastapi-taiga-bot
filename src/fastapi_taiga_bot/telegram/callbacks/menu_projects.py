...
class MenuProjectsCallback(TelegramCallback):
    data = "menu:projects"
    ...
    async def handle(self, update: TelegramUpdate) -> None:
        ...
        try:
            ...
            menu = self._menu_content.build_projects_menu(names)
...
def get_menu_projects_callback(
...
) -> MenuProjectsCallback:
    return MenuProjectsCallback(client, menu_content, auth_service, session)
